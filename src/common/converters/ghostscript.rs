...
pub fn file_to_file(path: &str, options: &GhostscriptConverterOptions) -> Result<String, String> {
    ...
    let converted_path = format!("{}.converted", path);
...
fn run(
    ...
    let mut command = Command::new(options.command.unwrap_or("gs"));
    ...
    if !output.status.success() {
        return Err(format!("Ghostscript exit with code {}", output.status.code().unwrap_or(1)));
    }
